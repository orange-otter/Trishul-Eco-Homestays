...
const renderMarkdown = (content: string) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('# ')) {
      ...
      flushList();
      renderedElements.push(
        <h2 key={`h2-${i}`} className="text-xl md:text-2xl font-bold text-stone-900 dark:text-stone-100 mt-6 mb-3 font-serif border-b border-stone-200 dark:border-gray-800 pb-2 leading-snug">
      ...
      currentList.push(
        <li key={`li-${i}`} className="leading-relaxed pl-1 text-stone-700 dark:text-stone-300">
  ...
  return (
    <div className="bg-white dark:bg-gray-900 p-8 rounded-2xl border border-stone-200 dark:border-gray-800 hover:shadow-lg transition-all duration-300">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold font-serif text-stone-900 dark:text-white tracking-wide">Trishul AI Assistant</h2>
      ...
      <div className="p-1 bg-stone-50 dark:bg-gray-800 rounded-xl flex gap-1 mb-6 max-w-md border border-stone-200/50 dark:border-gray-700/50">
        <button
          type="button"
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-semibold tracking-wide transition-all duration-300 cursor-pointer ${
            taskType === 'itinerary'
              ...
              : 'text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-200 hover:bg-stone-100/50 dark:hover:bg-gray-700/50'
          }`}
        >
